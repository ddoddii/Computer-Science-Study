...
ConcurrentHashMap은 자바에서 제공하는 스레드 안전한 맵 구현체입니다. 내부적으로 락을 사용하여 스레드 안전성을 보장합니다.
- **버킷이 비어있는 경우, 새로운 노드를 추가할 때는 어떻게 락을 걸지 않고 thread-safe 를 보장할 수 있나요?**
  - 내부적으로 CAS 연산을 사용하여 원자적인 연산을 수행하며 버킷에 새로운 노드를 추가합니다. 여러 스레드가 동시에 버킷에 접근해도 정확한 결과를 보장하므로 락을 걸지 않고도 스레드 안전성을 유지할 수 있습니다.
